...
class ProShootingAnalyzer:
    ...
    def run(self, video_path, output_path):
        ...
        if not self.saved_tracks:
            return {"score": 0, "speed": 0, "comment": "Error: 怨듭쓽 沅ㅼ쟻??李얠? 紐삵뻽?듬땲??"}
        ...
        if len(self.trajectory_3d) < 5:
            return {"score": 0, "speed": 0, "comment": "Error: 怨듭쓽 沅ㅼ쟻??異⑸텇??異붿쟻?섏? 紐삵뻽?듬땲??"}
        ...
        # --- Dynamic Comments ---
        s_tier = ""
        if final_speed < 60: s_tier = "?꾩돩???띾룄"
        elif final_speed < 90: s_tier = "媛뺣젰???덊똿"
        else: s_tier = "?꾨줈湲??뚯썙"
        ...
        if best_goal_cnt is None:
            ...
            r_tier = "吏留?怨⑤?瑜?踰쀬뼱?щ꽕???쁾"
        else:
            if dist_to_corner_score > 60:
                r_tier = ", 洹몃━怨??꾨꼍??援ъ꽍 沅ㅼ쟻?낅땲?? ?렞"
            else:
                r_tier = ", 怨⑥씤?낅땲?? ?럦"
        ...
        if final_speed > 100 and is_goal:
            comment = "??? ?먰씎誘??좎닔媛 ?멸퀬 媛??꾨꼍???덊똿?낅땲?? ?룇"
